...
async function runFullProtocolLoop() {
    console.log("================================================================");
    console.log("   ARC AGENT ECONOMY - FULL ON-CHAIN PRODUCTION LOOP");
    console.log("================================================================");
    ...
    try {
        ...
        // 2. Fuel
        console.log("\n[2/8] Fueling Agents (5.0 USDC each)...");
        await axios.get(`${HUB_URL}/admin/fuel-agent/${b.address}?amount=5.0`);
        await axios.get(`${HUB_URL}/admin/fuel-agent/${s.address}?amount=5.0`);
        await axios.get(`${HUB_URL}/admin/fuel-agent/${v.address}?amount=5.0`);
        console.log(">> Waiting for network inclusion (25s)...");
        await new Promise(r => setTimeout(r, 25000));

        // 3. Register & Stake
        console.log("\n[3/8] Registering Seller & Verifier (Stake: 1.0 USDC)...");
        await axios.post(`${HUB_URL}/execute/register`, {
            agentId: s.agentId, agentSecret: s.agentSecret,
            asSeller: true, asVerifier: false, stake: "1.0",
            capHash: "0x" + "1".repeat(64), pubKey: "0x" + "2".repeat(64)
        });
        await axios.post(`${HUB_URL}/execute/register`, {
            agentId: v.agentId, agentSecret: v.agentSecret,
            asSeller: false, asVerifier: true, stake: "1.0",
        ...
        console.log(`\n[4/8] Creating On-Chain Task ID: ${taskId} (Budget: 1.5 USDC)...`);
        ...
        // 5. Bid
        console.log("\n[5/8] Seller Bidding (1.0 USDC)...");
        await axios.post(`${HUB_URL}/execute/placeBid`, {
            agentId: s.agentId, agentSecret: s.agentSecret,
            taskId: taskId, price: "1.0", eta: 3600, meta: "0x" + "6".repeat(64)
        });
        await new Promise(r => setTimeout(r, 15000));
        ...
        // 8. Approve & Finalize
        console.log("\n[8/8] Verifier Approving & Buyer Finalizing...");
        ...
        await axios.post(`${HUB_URL}/execute/finalize`, {
            agentId: b.agentId, agentSecret: b.agentSecret,
            taskId: taskId
        });

        console.log("\n✅ FULL ON-CHAIN PROTOCOL COMPLETE!");
        ...
        console.error("!! Loop Failed:", err.response?.data || err.message);
    }
}
