...
async function main() {
    ...
    console.log("🦅 Saske: Checking all tasks for verifier assignments...");

    for (let i = 1n; i <= 31n; i++) {
        try {
            ...
            if (isV) {
                console.log(`Task #${i}: State=${stateStr}. I AM assigned as a verifier.`);
            } else if (task[12] === 3) { // SUBMITTED
                console.log(`Task #${i}: State=SUBMITTED. I am NOT a verifier.`);
            }
        } catch (err) {}
    }
}
