...
async function onboardAndFundAgent(agentName) {
    ...
    console.log(`   >> Depositing 3.45 USDC into Gateway (to cover Staking & Trading)...`);
    await axios.post(`${HUB_URL}/agent/gateway-deposit`, {
        agentName, agentSecret, amount: "3.45"
    }, { timeout: 120000 });
...
async function runA2ADemo() {
    ...
    const server = app.listen(TRADER_PORT, async () => {
        ...
        try {
            console.log(`   >> Staking 3.00 USDC to Sovereign Hub to register A2A Service...`);
            ...
            console.log(`   ✅ 3.00 USDC STAKED! Registered on the Sovereign Hub A2A Marketplace!\n`);
        } catch (err) {
