...
async function bootstrap() {
    try {
        ...
        // --- SEED A2A AGENT: DeFi Sentiment Analyst ---
        (async () => {
            try {
                if (mongoPromise) await mongoPromise;
                const db = mongoClient.db("arc_swarm");
                const agentName = "DeFi_Sentiment_Analyst";
                
                let agentDoc = await db.collection("agents").findOne({ agentName });
                let walletId, address;
                
                if (!agentDoc) {
                    console.log(`>> [SEED] Registering DeFi Sentiment Analyst agent...`);
                    const walletSetId = process.env.WALLET_SET_ID;
                    const walletResp = await client.createWallet({
                        walletSetId,
                        blockchain: "ARC-TESTNET"
                    });
                    walletId = walletResp.data.wallet.id;
                    address = walletResp.data.wallet.address;
                    const rawSecret = crypto.randomBytes(16).toString('hex');
                    
                    await saveWalletId(agentName, walletId, rawSecret, address, "0x0000000000000000000000000000000000000000");
                    console.log(`>> [SEED] Agent onboarded: ${address}`);
                    
                    agentDoc = await db.collection("agents").findOne({ agentName });
                } else {
                    walletId = agentDoc.walletId;
                    address = agentDoc.address;
                }
                
                // Pre-authorize slashing via EIP-712 BurnIntent (Upfront slashCheck)
                let slashCheck = null;
                try {
                    ...
                    const typedData = {
                        ...
                        message: (() => {
                            ...
                            intent.spec.sourceSigner = padToBytes32(address);
                            intent.spec.sourceDepositor = padToBytes32(address);
                            return intent;
                        })()
                    };
                    ...
                    if (signResp.data && signResp.data.signature) {
                        ...
                        console.log(">> [SEED] Secured upfront slashCheck for DeFi Sentiment Analyst");
                    }
                } catch (signErr) {
                    console.warn(">> [SEED] Upfront slashCheck signing failed (normal in mock/partial boot):", signErr.message);
                }

                // Register service in dynamic A2A registry
                const serviceUrl = "api/market-sentiment-analysis";
                const existing = a2aRegistry.find(s => s.url === serviceUrl);
                if (!existing) {
                    a2aRegistry.push({
                        id: 'a2a-sentiment-analyst',
                        name: agentName,
                        url: serviceUrl,
                        price: 0.03,
                        description: "Combines real-time Crypto Market Data and LLM Reasoning (Llama-3) to produce actionable DeFi arbitrage guide reports.",
                        slashCheck,
                        ratings: [],
                        averageRating: 5.0,
                        totalRatings: 0,
                    ...
                    console.log(`>> [SEED] DeFi Sentiment Analyst service registered in catalog.`);
                }
            } catch (err) {
                console.error(">> [SEED ERROR] Failed to seed DeFi Sentiment Analyst:", err.message);
            }
        })();
    } catch (e) {
...
// 3b. Pay-Per-Analysis — DeFi Sentiment & Reasoning Analysis
...
Perform a short, high-fidelity market sentiment analysis (bullish/bearish/neutral) and provide 2 actionable trading insights for an autonomous arbitrage bot. Respond in a clean markdown format.`;
            ...
            persistLedgerEntry({
                ...
                price: 0.03,
                provider: "DeFi Sentiment Analyst",
                payloadPreview: `Coin: ${coinName} | Price: $${price} | Analysis: ${output.substring(0, 50)}...`,
                timestamp: new Date().toISOString()
            });
            ...
            res.status(502).json({ success: false, error: "Service error: " + e.message });
        }
    }
);
