...
async function main() {
    ...
    for (let i = 1n; i <= 20n; i++) {
        try {
            ...
            const price = t[2].toString();
            
            if (state !== 0) {
                console.log(`Task #${i}: State=${state}, ApprovalTs=${approvalTs}, Price=${price}`);
                if (state === 4) {
                    const timeRemaining = (approvalTs + 3600) - now;
                    if (timeRemaining > 0) {
                        console.log(`   ⏳ Cool-off ends in ${timeRemaining}s`);
                    } else {
            ...
            // console.error(`Error checking Task #${i}: ${e.message}`);
        }
    }
}
